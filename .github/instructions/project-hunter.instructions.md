...
- `docs/VALIDATION_STAGE_CONTRACT.md`, and its machine-readable form `docs/VALIDATION_STAGE_CONTRACT.json`, define which boundary owns which proof. No stage may re-establish what another stage already established for the same immutable content.
- While implementing, verify with focused work: the affected tests plus the relevant lint and type checks. Do not repeat a repository-wide validation on an unchanged HEAD for reassurance.
- Push through the pre-push hook rather than rehearsing it by hand. It requires a clean tree, binds the pushed branch SHA to the checked-out exact HEAD, validates writer provenance and connector receipt freshness, runs the deterministic push-safety gates (Architecture Index Guard, Artifact Guard, Defect Prevention Guard, Ruff, Black, Mypy), and rechecks HEAD/tree state before authorizing the network push.
- The authoritative full repository proof is the hosted exact-head `Hunter / Pre-PR Preflight` run of `python scripts/hunter_pr_preflight.py --mode normal`, where Architecture Index Guard, Artifact Guard, Defect Prevention Guard, Ruff, Black, Mypy, and Pytest must all pass. Candidate admission fails closed without it. If you run that full lane locally, use `--record-receipt --reuse-receipt` so one immutable identity is never validated twice.
- A push reporting `Everything up-to-date` mutates nothing and needs no proof. Never launch a synthetic validation run for it, and never create an empty commit to manufacture a new validation identity.
...
- For an intentional tests-first RED commit, create `.hunter-preflight-mode` containing exactly `tests-first-red` and commit that marker on the exact branch head being pushed. The exact-head preflight may pass only when the Architecture Index Guard, Artifact Guard, Defect Prevention Guard, Ruff, Black, and Mypy are green and Pytest is genuinely RED; because that RED result is the proof being made, this is the one lane where Pytest still runs at the push boundary. This exception is for Draft tests-first work only; it never makes failing tests merge-ready.
