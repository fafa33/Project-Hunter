...
def test_status_still_validates_manifest_against_a_pristine_root(
    ...
    """Regression guard for a review finding on the fix above: skipping repository
    construction when no database exists yet must not also skip validating the
    manifest's own required fields. A malformed status manifest (here, a
    non-ISO-8601 `known_by`) against a pristine application root must still raise,
    not be masked as `available: False`, and must still not create the database."""
    manifest = _write(
        tmp_path,
