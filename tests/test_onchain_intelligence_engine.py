...
def onchain_records():
    return (
        AddressSnapshot(**base("addr-old", 120), active_addresses=1_000, new_addresses=120, returning_addresses=500, retained_addresses=450, sybil_ratio=0.05, bot_ratio=0.04),
        AddressSnapshot(**base("addr-new", 1), active_addresses=1_600, new_addresses=240, returning_addresses=900, retained_addresses=850, sybil_ratio=0.06, bot_ratio=0.05),
        TransactionSnapshot(**base("tx-old", 120), transaction_count=10_000, adjusted_transaction_count=7_000, gross_volume=1_000_000, adjusted_volume=700_000, low_value_ratio=0.08, repeated_pattern_ratio=0.07),
        TransactionSnapshot(**base("tx-new", 1), transaction_count=16_000, adjusted_transaction_count=11_000, gross_volume=1_800_000, adjusted_volume=1_300_000, low_value_ratio=0.09, repeated_pattern_ratio=0.08),
        TransferSnapshot(**base("transfer-new", 1), transfer_value=1_200_000, adjusted_transfer_value=1_000_000, internal_transfer_ratio=0.1, circular_transfer_ratio=0.08),
        CapitalFlowSnapshot(**base("capital-new", 1), inflow=1_000_000, outflow=400_000, retained_capital=750_000, circular_flow_ratio=0.07, internal_flow_ratio=0.08),
        ExchangeFlowSnapshot(**base("exchange-new", 1), inflow=200_000, outflow=500_000, exchange_label_quality=0.8, redistribution_ratio=0.1),
        BridgeFlowSnapshot(**base("bridge-old", 120, chain="base"), inflow=100_000, outflow=60_000, bridge_label_quality=0.8, pass_through_ratio=0.08, source_chain="ethereum", target_chain="base"),
        BridgeFlowSnapshot(**base("bridge-new", 1, chain="base"), inflow=240_000, outflow=80_000, bridge_label_quality=0.8, pass_through_ratio=0.1, source_chain="ethereum", target_chain="base"),
        StakingFlowSnapshot(**base("staking-new", 1), staked_inflow=300_000, staked_outflow=90_000, unstaked=30_000),
        HolderSnapshot(**base("holder-old", 120), holder_count=20_000, retained_holders=10_000, long_term_holders=5_000, top_holder_share=0.32, dormant_supply_ratio=0.42, active_supply_ratio=0.58, accumulation_wallets=2_000, distribution_wallets=900),
        HolderSnapshot(**base("holder-new", 1), holder_count=28_000, retained_holders=18_000, long_term_holders=9_000, top_holder_share=0.28, dormant_supply_ratio=0.46, active_supply_ratio=0.54, accumulation_wallets=4_500, distribution_wallets=1_100),
        SupplyDistributionSnapshot(**base("supply-new", 1), circulating_supply=100_000_000, top_10_share=0.35, top_100_share=0.55, gini=0.4, distribution_quality=0.68),
        ContractActivitySnapshot(**base("contract-old", 120), active_contracts=50, interactions=5_000, unique_callers=1_000, protocol_owned_ratio=0.3, spam_contract_ratio=0.08, generated_contract_ratio=0.06, contract_address="0xcore"),
        ContractActivitySnapshot(**base("contract-new", 1), active_contracts=80, interactions=9_000, unique_callers=2_200, protocol_owned_ratio=0.28, spam_contract_ratio=0.07, generated_contract_ratio=0.06, contract_address="0xcore2"),
        ...
        ApplicationActivitySnapshot(**base("app-core", 1), application_id="core", active_users=1_500, transaction_share=0.42, volume_share=0.4),
        ApplicationActivitySnapshot(**base("app-third", 1), application_id="third-party", active_users=700, transaction_share=0.25, volume_share=0.2),
        ...
        ValidatorDistributionSnapshot(**base("validator-new", 1), validator_count=80, top_validator_share=0.22, staker_count=4_000, staker_concentration=0.25),
        GovernanceActivitySnapshot(**base("governance-new", 1), proposals=4, voters=1_200, participation_ratio=0.48),
        OnchainEvent(**base("event-new", 1), event_type="upgrade", severity=0.3, description="Routine contract upgrade."),
    )
...
def test_anomaly_assessment_detects_suspected_abnormal_activity() -> None:
    noisy = OnchainNormalizer().normalize(
        (
            ...
            TransactionSnapshot(**base("wash", 1), transaction_count=1_000, adjusted_transaction_count=100, gross_volume=1000, adjusted_volume=100, low_value_ratio=0.8, repeated_pattern_ratio=0.7, gas_anomaly_ratio=0.6),
