"""Trusted-bot comment policy for Hunter Merge Readiness.

The policy is fail-closed by default: every top-level PR Conversation comment
blocks readiness until the repository owner acknowledges that exact comment
version with a 👍 reaction. Exactly one narrow exemption exists -- structurally
identifiable status/advisory comments emitted by trusted repository automation --
and it is identified by a marker in the comment body, never by "it is a bot".

Under the current-state reconciler this policy has a second, stronger
consequence: an exempt comment is filtered out before the state snapshot is
built, so it cannot move the semantic revision either. Trusted automation
posting an advisory comment is therefore not merely non-blocking, it is not a
semantic event at all.
"""
...
def test_unknown_bot_carrying_a_known_marker_is_not_exempt():
    """The marker alone must never confer exemption; the author identity gates it."""
