...
def evidence_for(issue: preflight.IssueIdentity) -> dict[str, dict[str, str]]:
    return {
        criterion: {"status": "PASS", "evidence": f"deterministic proof for criterion {index}"}
...
def test_issue_acceptance_criteria_are_extracted_only_from_canonical_section() -> None:
    issue = fixture_issue()

    criteria = preflight.issue_acceptance_criteria(issue.body)

    assert criteria == (
        "A deterministic canonical preflight command/API exists.",
        "PR creation can produce a complete canonical PR body from the verified Issue and repository template.",
        "Human approval remains required for merge.",
    )


def test_generator_uses_issue_template_scope_and_complete_matrix() -> None:
    issue = fixture_issue()
    body = generated_body()
    ...
    assert "scripts/hunter_governance_preflight.py" in body
    assert "APPROVED" not in body


def test_generator_defaults_unproven_criteria_to_blocked_and_changes_required() -> None:
...
def test_pr_body_validation_rejects_missing_matrix() -> None:
    issue = fixture_issue()
...
def test_pr_body_validation_rejects_omitted_issue_criterion() -> None:
    issue = fixture_issue()
    body = generated_body()
    row = preflight.parse_acceptance_matrix(body)[0]
    body = body.replace(
        f"| {row.criterion} | PASS | {row.evidence} |\n",
        "",
    )

    with pytest.raises(preflight.PreflightError, match="omits governing Issue acceptance criteria"):
        preflight.validate_pr_body(body, issue, head_sha=HEAD, base_sha=BASE, promotion=False)


def test_pr_body_validation_rejects_extra_invented_criterion() -> None:
    issue = fixture_issue()
    body = generated_body().replace(
        "|---|---|---|\n",
...
def test_pass_cannot_be_inferred_from_green_ci_only() -> None:
    issue = fixture_issue()
    body = generated_body()
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda body: body.replace("- [x] `READY FOR REVIEW`", "- [ ] `READY FOR REVIEW`"),
            "Exactly one",
        ),
        (
            lambda body: body.replace("- [ ] `BLOCKED`", "- [x] `BLOCKED`"),
            "Exactly one",
        ),
        (
...
def test_ready_promotion_rejects_zero_multiple_or_non_ready_declarations(body_transform, message: str) -> None:
    issue = fixture_issue()
    body = body_transform(generated_body())
...
def test_ready_promotion_rejects_blocked_criterion() -> None:
    issue = fixture_issue()
    body = generated_body()
    first = preflight.parse_acceptance_matrix(body)[0]
...
def test_exact_head_marker_rejects_new_source_commit() -> None:
    issue = fixture_issue()

    with pytest.raises(preflight.PreflightError, match="stale relative to the current source head"):
        preflight.validate_pr_body(generated_body(), issue, head_sha="c" * 40, base_sha=BASE, promotion=True)


def test_exact_pair_marker_rejects_target_advance() -> None:
    issue = fixture_issue()

    with pytest.raises(preflight.PreflightError, match="stale relative to the current target revision"):
        preflight.validate_pr_body(generated_body(), issue, head_sha=HEAD, base_sha="d" * 40, promotion=True)


def test_rule_21_rejects_issue_branch_commit_title_and_body_identity_mismatch() -> None:
    issue = fixture_issue()
...
def test_rule_21_rejects_closed_issue(tmp_path: Path) -> None:
    ...
    issue = preflight.IssueIdentity.from_payload("fafa33/Project-Hunter", payload)
    temporary = tmp_path / "closed_issue.json"
    temporary.write_text(json.dumps(payload), encoding="utf-8")

    assert issue.state == "closed"
    with pytest.raises(preflight.PreflightError, match="not open"):
        preflight.load_issue(issue.repository, issue.number, temporary)


def test_ownership_guard_rejects_review_classification_semantics_in_implementation_contract() -> None:
    added = {
...
def test_ownership_guard_allows_reference_to_canonical_review_owner() -> None:
    added = {
        "docs/HUNTER_IMPLEMENTATION_CONTRACT.md": [
...
def test_diff_parser_only_returns_added_content_by_file() -> None:
...
+newer
"""
    assert preflight.added_lines_from_diff(diff) == {
        "docs/A.md": ["new"],
        "docs/B.md": ["newer"],
    }


def test_systemic_blocking_finding_cannot_resolve_without_durable_guard_and_verifier_evidence() -> None:
    finding = preflight.FindingResolution(
        finding_id="P2-1",
        severity="blocking",
        classification="systemic",
        classification_evidence="same defect can recur at a reusable boundary",
        reusable_boundary="PR body generator",
    ...
    with pytest.raises(preflight.PreflightError, match="durable reusable hardening"):
        preflight.validate_finding_resolution(finding)


def test_systemic_finding_resolution_accepts_complete_canonical_evidence() -> None:
    finding = preflight.FindingResolution(
        finding_id="P2-1",
        severity="blocking",
        classification="systemic",
        classification_evidence="same defect can recur at a reusable boundary",
        reusable_boundary="PR body generator",
        durable_guard_evidence="generator regression test rejects the defect class",
        verifier_evidence="independent verifier reproduced the counterfactual failure",
        resolved=True,
    )
...
def test_ready_live_preflight_rejects_unresolved_review_thread(monkeypatch) -> None:
    issue = fixture_issue()
    ...
    monkeypatch.setattr(readiness, "feedback_error", lambda current: "Unresolved review threads remain: 1.")
    monkeypatch.setattr(preflight, "_gh_json", lambda _args: {"head": {"ref": "governance/issue-276-agent-preflight"}})
    monkeypatch.setattr(preflight, "_run", lambda _args, cwd=None: "token")
...
def test_trace_helper_is_safe_for_merge_readiness_current_state() -> None:
    body = generated_body()

    assert preflight.validate_trace_against_state(body, head_sha=HEAD, base_sha=BASE) is None
    assert "source head" in preflight.validate_trace_against_state(body, head_sha="c" * 40, base_sha=BASE)
