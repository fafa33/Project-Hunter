...
def _harness() -> ModuleType:
    """Load the test-only conformance adapter.

    The future implementation contribution may expose any compliant production API.
    It must add tests/source_handling_runtime_harness.py as a thin adapter from that
    production API to the semantic operations exercised here. This keeps the frozen
    contract tests behavioral instead of freezing arbitrary product module/class names.

    This tests-first PR is intentionally RED while the adapter/runtime do not exist.
    Do not skip, xfail, or import-skip this path to obtain green CI.
    """
    ...
    spec = importlib.util.spec_from_file_location("source_handling_runtime_harness", HARNESS)
...
def test_authorization_rule_v1_fixture_is_closed_and_has_reviewed_golden_digest() -> None:
    ...
    for forbidden in body["forbidden_permission_methods"]:
        assert matrix[forbidden] == []


def test_fixture_tamper_changes_golden_digest() -> None:
    payload = _rule_fixture()
    payload["rule_body"]["allow_second_genesis"] = True
    assert _canonical_sha256(payload) != GOLDEN_AUTH_RULE_V1_SHA256
...
def test_tampered_authorized_payload_digest_rejects_at_publication_and_persistence() -> None:
    ...
    with pytest.raises(h.blocked_error):
        h.verify_publication(auth, "FIELD_CATEGORY_REGISTRY", "registry:v1", payload)
    with pytest.raises(h.blocked_error):
        h.validate_durable_payload(
            decision={"publication_authorization": auth, "field_category_registry_id": "registry-v1"},
...
def test_authority_publication_without_governed_authorization_rejects(publication_kind: str) -> None:
    h = _harness()
    ...
    with pytest.raises(h.blocked_error):
        store.publish(
            ...
            record={"id": "candidate", "publication_authorization": None},
        )
...
def test_repository_direct_write_cannot_bypass_publication_authority() -> None:
    ...
    store = h.authority_store()
    with pytest.raises(h.blocked_error):
        store.direct_write(family="FACT", scope="doc-1", record={"id": "forged-fact"})
...
def test_non_authoritative_evidence_cannot_grant_or_release_permission(strength: str, method: str) -> None:
    h = _harness()
    for requested_change in ("PERMISSIVE_GENESIS", "LESS_RESTRICTIVE", "RESTRICTION_RELEASE"):
        with pytest.raises(h.blocked_error):
...
def test_bootstrap_digest_mismatch_and_second_genesis_reject() -> None:
    ...
    store = h.authority_store()
    bad = _rule_fixture()
    bad["rule_body"]["allow_second_genesis"] = True
    with pytest.raises(h.blocked_error):
        h.publish_genesis_rule(store, bad, expected_golden_sha256=GOLDEN_AUTH_RULE_V1_SHA256)
    h.publish_genesis_rule(store, _rule_fixture(), expected_golden_sha256=GOLDEN_AUTH_RULE_V1_SHA256)
    with pytest.raises(h.blocked_error):
        h.publish_genesis_rule(store, _rule_fixture(), expected_golden_sha256=GOLDEN_AUTH_RULE_V1_SHA256)
...
def test_successor_rule_cannot_self_authorize_or_use_later_current_rule() -> None:
    ...
    with pytest.raises(h.blocked_error):
        h.publish_successor_rule(store, successor, authorizing_rule_id="AUTHORIZATION_RULE_V2")
...
def test_strict_known_multiple_heads_block_and_never_use_tiebreakers() -> None:
    ...
    cutoff = _utc("2026-08-14T12:00:00Z")
    records = [
        {"id": "a", "scope": "doc-1", "supersedes": None, **_base_times()},
        {"id": "b", "scope": "doc-1", "supersedes": "a", **_base_times()},
        {"id": "c", "scope": "doc-1", "supersedes": "a", **_base_times()},
    ]
    with pytest.raises(h.blocked_error):
        h.strict_known_head(records, cutoff=cutoff, scope="doc-1")
...
def test_fact_join_is_associative_commutative_idempotent_and_unknown_is_absorbing() -> None:
    h = _harness()
    a = {"sensitivity": "PUBLIC", "operation_restrictions": set(), "persistence_restriction": "FULL_CONTENT_ALLOWED", "secret_presence": set(), "operation_restrictions_known": True, "secret_presence_known": True, "withdrawn": False, "deleted_at_source": False, "historically_unavailable": False, "availability_known": True}
    b = {**a, "sensitivity": "CONFIDENTIAL", "operation_restrictions": {"ACCESS_RESTRICTED"}}
    c = {**a, "persistence_restriction": "METADATA_ONLY", "secret_presence": {"SECRET_PRESENT"}}
    ...
    assert h.restrictive_fact_join([h.restrictive_fact_join([a, b]), c]) == h.restrictive_fact_join([a, h.restrictive_fact_join([b, c])])
    for unknown in (
...
def test_historical_registry_identity_cannot_be_substituted_by_current_registry() -> None:
    ...
    decision = {
        ...
        "durable_dispositions": {"AUDIT_FIELD": {"PERSIST": "DENY"}},
    }
    with pytest.raises(h.blocked_error):
        h.validate_durable_payload(
            decision=decision,
            registry={"field_category_registry_id": "registry-v2", "field_map": {"audit": ["AUDIT_FIELD"]}},
...
def test_multicategory_field_uses_most_restrictive_operation_specific_disposition() -> None:
    ...
    registry = {
        "field_category_registry_id": "registry-v1",
        "field_map": {"x": ["OPERATIONAL_METADATA", "AUDIT_FIELD"]},
    }
    decision = {
        "field_category_registry_id": "registry-v1",
        "durable_dispositions": {
            "OPERATIONAL_METADATA": {"PERSIST": "ALLOW"},
            "AUDIT_FIELD": {"PERSIST": "DENY"},
        },
    }
    with pytest.raises(h.blocked_error):
        h.validate_durable_payload(decision=decision, registry=registry, payload={"x": "value"}, secret_presence=set())
...
def test_lifecycle_join_is_associative_commutative_idempotent_and_invalid_values_block() -> None:
    ...
    for a in values:
        for b in values:
            ...
            for c in values:
                assert h.lifecycle_join([h.lifecycle_join([a, b]), c]) == h.lifecycle_join([a, h.lifecycle_join([b, c])])
    with pytest.raises(h.blocked_error):
...
def test_secret_or_credential_derived_representation_is_structurally_non_persistable(field: str) -> None:
    h = _harness()
    registry = {
        "field_category_registry_id": "registry-v1",
    ...
    for marker in ({"SECRET_PRESENT"}, {"CREDENTIAL_PRESENT"}):
        with pytest.raises(h.blocked_error):
            h.validate_durable_payload(
                decision=decision,
                registry=registry,
                payload={field: {"value": "protected-material", "derived_from_protected_content": True}},
...
def test_safe_control_metadata_is_not_banned_merely_because_source_contains_secret() -> None:
    ...
    registry = {
        "field_category_registry_id": "registry-v1",
        "field_map": {"run_id": ["SAFE_CONTROL_ID"]},
    }
    decision = {
        "field_category_registry_id": "registry-v1",
        "durable_dispositions": {"SAFE_CONTROL_ID": {"PERSIST": "ALLOW"}},
    }
    h.validate_durable_payload(
        decision=decision,
        registry=registry,
        payload={"run_id": {"value": "run-123", "derived_from_protected_content": False}},
        secret_presence={"SECRET_PRESENT"},
    )
...
def test_blocked_audit_cannot_leak_protected_content_or_source_derived_diagnostics() -> None:
    ...
    registry = {
        "field_category_registry_id": "registry-v1",
        "field_map": {"diagnostic": ["DIAGNOSTIC"], "audit": ["AUDIT_FIELD"]},
    }
    decision = {
        "field_category_registry_id": "registry-v1",
        "durable_dispositions": {
            "DIAGNOSTIC": {"PERSIST": "ALLOW"},
            "AUDIT_FIELD": {"PERSIST": "ALLOW"},
        },
    }
    with pytest.raises(h.blocked_error):
        h.validate_durable_payload(
            ...
            registry=registry,
            payload={
                "diagnostic": {"value": "secret appeared here", "derived_from_protected_content": True},
                "audit": {"value": "secret appeared here", "derived_from_protected_content": True},
            },
            secret_presence={"SECRET_PRESENT"},
        )
...
def test_legacy_absence_cannot_be_upgraded_into_historical_authority() -> None:
    ...
    assert not h.strict_known_eligible(migrated, _utc("2026-08-14T12:00:00Z"))
