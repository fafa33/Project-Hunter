...
Hunter needs the smallest coherent Context Intelligence and Prompt Intelligence foundation that:

1. deterministically constructs a model-independent context decision from governed inputs;
...
Hunter establishes an AI Context and Prompt Intelligence Foundation with four current stage owners and three future boundaries:

```text
AIExecutionIntent
    -> ContextResolver
    -> ContextSelector
    -> AIContextSelectionLedger
    -> ContextBudgetAllocator
    -> ContextAllocationResult + ContextPackage
    -> PromptCompiler
    -> PromptPlan + PromptArtifact
    -> AIPreModelBuildRecord
    -> future ModelAdapter
    -> future ResponseValidator
    -> proposal or recommendation consumer
    -> existing deterministic or human authority boundary
```

No single `AIService` may own this complete chain. The current foundation ends at an exact `PromptArtifact` and an immutable `AIPreModelBuildRecord`. That record is a subordinate build artifact linked to the initiating `PipelineRun`, workflow run, task identity, or equivalent existing execution owner. It never replaces Hunter's top-level execution identity and never owns provider attempts or responses. Model invocation, provider attempts, response semantics, and any full AI-interaction aggregate require later Model Adapter architecture.

## Architectural Boundaries

### ContextResolver

The resolver converts governed source coordinates into exact source references and resolution outcomes.

It owns:

- exact repository commit, document version, evidence record version, span, range, and content-hash resolution;
...
The selector applies one versioned `ContextSelectionPolicy` to resolved source references and an `AIExecutionIntent`.
...
The compiler converts a `PromptPlan` and finalized `ContextPackage` into an exact `PromptArtifact`.
...
A future `ModelAdapter` may consume a `PromptArtifact` and capability requirements. It may not modify upstream intent, source resolution, context selection, or prompt content without producing a new, separately identified build.

A future `ResponseValidator` may validate syntax, schema, prohibited capabilities, evidence references, and consumer-specific proposal eligibility. Validation does not promote AI output to canonical authority.

An AI consumer may convert a validated response into a domain proposal or advisory recommendation only through its own authorized service boundary. Canonical promotion remains governed by the owning accepted ADR and never by this foundation.

## Canonical Concepts and Contracts

The names below are canonical architectural concepts. Exact language types, module paths, and SQL layouts are implementation details.

### AIExecutionIntent

`AIExecutionIntent` is the provider-independent reason and authority envelope for model-facing work. It must identify:

- task type and bounded task objective;
- initiating consumer and workflow stage;
- target identity and exact temporal or repository scope where applicable;
- expected output-contract identity and version;
- requested output-authority ceiling, which is limited to proposal or recommendation classifications already available to the authorized initiating consumer;
...
The authority-ceiling field records a requested maximum; it grants no authority by its presence. The initiating service must already be authorized to request that class of work, and the later deterministic or human authority boundary independently decides what, if anything, may be acted upon or promoted. The intent must never name a preferred provider as architectural authority. Operational capability requirements may be declared without selecting a provider.

### ContextSourceReference and resolved source views

`ContextSourceReference` identifies a source without duplicating the source owner's canonical record. When an `EvidenceSpan` already exists, the context reference points to its stable identity and records only the additional AI-context coordinates.

Every reference must carry, as applicable:

- source class and canonical owner;
- logical identity and exact record or artifact identity/version;
...
A resolver may expose an immutable resolved view for selection and budgeting, but this ADR does not establish a generic `ContextItem` record family. For the first consumer, the view references exact `EvidenceSpan` records and adds only AI-context decision metadata. Future consumers may introduce a reusable resolved-view contract only after consumer evidence demonstrates that it does not duplicate an existing source owner's record.

### ContextSelectionPolicy and ContextSelectionPlan

A `ContextSelectionPolicy` is a governed specification with one logical identity, explicit semantic version, content hash, and schema version. It declares:

- eligible source classes and resolvers;
...
A `ContextSelectionPlan` binds one intent, one policy version, and exact resolver inputs before resolution begins. It prevents callers or prompt templates from silently expanding source scope during execution.
...
`AIContextSelectionLedger` is the model-independent, AI-specific decision ledger for source resolution and selection. Its explicit name and namespace distinguish it from Hunter Governance Review's existing `ContextManifest`; neither contract implements, imports, replaces, or extends the other.
...
### ContextAllocationResult and ContextPackage

`ContextAllocationResult` applies one minimal, versioned capability constraint and one budget-policy version to one `AIContextSelectionLedger`. It records per-item inclusion or budget exclusion, capacity accounting, coverage outcome, reason codes, and its own deterministic capability-specific identity.

`ContextPackage` contains only the finalized ordered items authorized for prompt rendering by one allocation result. Its identity is derived from the allocation-result identity, finalized source identities, exact content hashes/ranges, order, applicable policy identities, and schema version. It cannot prove that a model reviewed the content; review requires a later successful invocation record linked to this exact package and prompt artifact.
...
`PromptPlan` is a provider-independent semantic representation, not a concatenated string. It contains ordered, typed sections such as:
...
`PromptArtifact` is the exact canonical pre-model payload handed to a future Model Adapter. It contains, and durably persists only when governing source policy permits:
...
- PromptPlan identity;
- `AIContextSelectionLedger`, `ContextAllocationResult`, and ContextPackage identities;
...
Provider-specific request wrapping, authentication, transport headers, and provider parameters are not part of `PromptArtifact`; they belong to later Model Adapter architecture. If a future adapter transforms model-facing message content, the transformed exact payload must be recorded as a distinct provider-request artifact and may not be treated as the original prompt artifact. If policy prohibits durable exact-byte retention, the build records reconstruction unavailability rather than treating hashes or regenerated current content as an exact artifact history.

### AIPreModelBuildRecord

`AIPreModelBuildRecord` is the immutable record of one completed or failed pre-model build. It identifies:
...
- `AIContextSelectionLedger` identity;
- `ContextAllocationResult` and ContextPackage identities when allocation completed;
- PromptPlan and PromptArtifact identities when compilation completed;
...
- ContextPackage content and order;
- budget calculations and outcome;
- PromptPlan;
- PromptArtifact roles, content bytes, and hashes; and
...
Provider or model identity alone must not alter intent, source-resolution, selection-plan, or `AIContextSelectionLedger` identity. A different explicit capability constraint may legitimately produce a different `ContextAllocationResult`, ContextPackage, PromptPlan build, PromptArtifact, and `AIPreModelBuildRecord`; it must not rewrite historical resolution or selection truth.
...
equal canonical intent
    ...
    + equal AIContextSelectionLedger
    ...
    -> equal canonical PromptArtifact bytes and digest
```

### Model non-determinism

A model response is an observed execution result. Temperature zero, a provider seed, or a provider deterministic-mode claim does not convert it into deterministic Hunter authority. Re-executing the same PromptArtifact creates a new attempt and response record.

### Post-model deterministic validation

Response schema validation, prohibited-capability checks, evidence-reference checks, and consumer eligibility rules must be deterministic for fixed response bytes and validator versions. A valid response remains a proposal or recommendation unless a separate canonical service is authorized to consume it.
...
- `AIExecutionIntent` has a deterministic specification identity derived from its semantic content. An operational request may separately reference that identity with an attempt correlation record.
- `AIPreModelBuildRecord` has a deterministic build identity, references the existing initiating run/workflow/task owner, and has no operational-attempt or response ownership.
...
| `AIExecutionIntent` | None | Same intent identity. |
| Source-resolution inputs/results, `ContextSelectionPlan`, and `AIContextSelectionLedger` | None | Same resolution, plan, and ledger identities. |
| `ContextAllocationResult` and ContextPackage | Explicit versioned constraint | May differ. |
| PromptPlan build, PromptArtifact, and `AIPreModelBuildRecord` | Through the allocation result and explicit compiler constraint | May differ and receive distinct deterministic identities. |
...
| --- | --- | --- |
| Applicability and eligibility | `ContextSelector` applying `ContextSelectionPolicy` | `APPLICABLE_ELIGIBLE`, `NOT_APPLICABLE`, or `REJECTED`. Rejection records whether security, trust, authority, temporal, retention, or contract policy disallowed the coordinate. |
| Resolution outcome | `ContextResolver` | `RESOLVED`, `UNRESOLVED_REQUIRED`, `UNRESOLVED_OPTIONAL`, or `NOT_ATTEMPTED`. Resolution records exact content/provenance availability and never decides relevance or budget inclusion. |
| Selection decision | `ContextSelector` | `SELECTED`, `NOT_SELECTED`, or `NOT_EVALUATED`. Selection is model/capability independent. |
| Allocation and inclusion | `ContextBudgetAllocator` | `INCLUDED`, `BUDGET_EXCLUDED`, or `NOT_ALLOCATED`. This dimension exists only in `ContextAllocationResult`, not in the model-independent ledger. |
...
`AIContextSelectionLedger` records the applicability, resolution, selection, and reason dimensions. `ContextAllocationResult` separately records allocation/inclusion and its reason. Invalid combinations fail closed and produce no successful ContextPackage.
...
Prompt compilation is a pure build step over a finalized ContextPackage and versioned PromptPlan specification.
...
AI output remains evidence, proposal, or recommendation. Successful schema validation proves only conformance to the response contract; it does not prove truth, correctness, sufficiency, or authority.
...
- Credentials and secrets must not be intentionally persisted in PromptArtifact history. Detection produces rejection or a fail-closed build according to policy; it does not authorize silent redaction followed by a claim of exact reconstruction.
...
3. `exact pre-model reconstruction`: reconstruct byte-identical PromptArtifact content from retained historical sources, ledger, allocation inputs, prompt specification, compiler identity/version, and canonicalization format/version.
...
6. `provider re-execution`: submit an existing PromptArtifact through a later adapter as a new operational attempt; response equality is not claimed.
...
Minimum persistable provenance, subject to governing source policy, includes intent, source identities/revisions/ranges/hashes, dimensional resolution and selection decisions, every non-inclusion reason, policy versions, allocation calculations, PromptPlan, exact PromptArtifact bytes/hashes when retainable, explicit reconstruction capability, and initiating run/workflow/task plus predecessor-build linkage. Later model integration must separately decide retention of provider/model identity, parameters, exact request, response, usage, failures, validation outcome, and attempt ordering.
...
The first consumer is Evidence Intelligence's existing provider seam. Migration is conceptual and separately implemented:

```text
ExtractionRequest
    -> AIExecutionIntent (proposal-only evidence extraction)
    -> exact EvidenceSpan references + ExtractionSchema reference
    -> fixed/versioned ContextSelectionPolicy + ContextSelectionPlan
    -> AIContextSelectionLedger
    -> one explicit versioned capability constraint
    -> ContextAllocationResult + ContextPackage
    -> PromptPlan
    -> PromptArtifact
    -> AIPreModelBuildRecord linked to the existing processing/workflow run identity
```
...
`ExtractionRequest` may be adapted as the Phase 1 input without changing its current contract. AI Context and Prompt Intelligence references or adapts `EvidenceSpan`, `ExtractionSchema`, `SecureAIProviderRunner`, `ExtractionProposal`, and deterministic claim validation/persistence; it does not replace their authority. `AIExtractionProvider` remains untouched until a later Model Adapter decision provides a migration path and must not be silently reinterpreted.
...
No Context Intelligence, Prompt Intelligence, PromptArtifact, provider, model, or response defined or anticipated here is required for the `Hunter Governance Review` status. Existing zero-LLM architecture regression tests remain mandatory.
...
This foundation leaves stable interfaces for later decisions:

- Model Adapter and Provider Routing consume exact PromptArtifacts plus capability requirements and produce immutable invocation/health/failure records. They do not own context or prompt policy.
- Model Capability Profiles describe versioned build and invocation constraints. They do not grant provider authority.
- Response Validation consumes exact response bytes, output contracts, context/provenance references, and validator versions. It emits validation evidence, not canonical truth.
- AI-assisted Review consumes execution/validation artifacts and emits advisory findings with exact coverage and authority limits.
- Review Engine 2.0, if ever authorized, composes advisory and deterministic evidence without placing provider availability in merge authority.
- Architectural Intelligence consumes immutable engineering/review evidence and proposes architecture changes; human/repository governance remains final authority.
- Historical experimentation may compare policies, prompts, providers, and responses through new build and later invocation records without rewriting historical truth or promoting experimental output.
...
- model-independent intent, resolution, plan, and `AIContextSelectionLedger` identities;
...
- equal canonical inputs reconstruct byte-identical PromptArtifact bytes and digest when retention policy permits;
...
- `AIPreModelBuildRecord` remains subordinate to the existing initiating execution/run/workflow owner and owns no attempts or responses;
...
| --- | --- |
| Unit | Legal/illegal context-dimension combinations; stable reason codes; deterministic ordering and allocation arithmetic; canonical Unicode/newline/path/mapping/collection/numeric/boolean/null/whitespace serialization; compiler and canonicalization version sensitivity; exact PromptArtifact digest; runtime timestamp exclusion. |
...
| Architecture regression | Governance `ContextManifest` remains separate from `AIContextSelectionLedger`; Hunter Governance Review remains LLM/provider independent; no second top-level execution ontology appears; prompt compilation has no arbitrary repository/network retrieval; model/provider configuration cannot become upstream source authority; forbidden mutation/tool capabilities remain outside the model boundary. |

## Consequences

- Hunter gains one canonical pre-model build boundary subordinate to existing execution ownership without prematurely selecting providers or storage technology.
...
- Consumers must define output authority and capability restrictions before requesting model-facing work.
...
- ADR 0030 remains Proposed and is neither accepted nor superseded. This ADR independently establishes the detailed Context and Prompt foundation. If ADR 0030 later proceeds to acceptance, its implementation sequence and overlapping contract language must be reconciled to treat this accepted decision as the detailed owner.
