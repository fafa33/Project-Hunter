...
| [0033](0033-source-handling-classification-authority.md) | Source Handling Classification Authority | Proposed | Assigns exclusive ownership of authoritative source-handling facts and governed retention-policy authority to the Evidence Intelligence consumer-side Source Handling Authority, closing only the authority gap ADR 0031 left open; binds safety invariants (caller/provider input is never authority, retainability is derived, simultaneous restrictions are not collapsed, unresolved or conflicting authority is `BLOCKED`, no permissive fallback), historical/replay invariants (immutable versioned authority, superseding corrections, cutoff-knowable replay, explicit absence, bytes are not proof of authority), and the persistence invariant (independently resolve, rederive, verify, reject; never trust caller-supplied classification, policy, decision, or decision identity); mechanics are deferred to a separate design and implementation contract; `EvidenceSpan` unchanged, `EvidenceContextSourceReference` not redefined, no analytical authority |
