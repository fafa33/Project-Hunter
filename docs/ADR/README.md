...
| [0028](0028-evidence-assembly-supporting-authorities.md) | Canonical Evidence Assembly Supporting Authorities | Proposed | Ownership, record family, persistence, versioning, correction, provenance, strict-known replay, conflict, and amendment-governance semantics for Methodology Contract (owned by `hunter.valuation_methodology`), Evidence Shape Registry, Evidence Semantics, and `EvidenceSemanticsClassificationRuleset` (all owned by `hunter.evidence_assembly`); narrow amendments to ADR 0025's ownership silence and to ADR 0021 (additive `accepts_assembled_evidence` field on `ValuationMethodologySnapshot`'s field-list table); ADR 0022's Milestone-2 permitted values reaffirmed, unmodified |
