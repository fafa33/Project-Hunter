...
| [0028](0028-evidence-assembly-supporting-authorities.md) | Canonical Evidence Assembly Supporting Authorities | Proposed (revision 5) | Establishes Methodology Contract and Evidence Shape Registry persistence boundaries plus one canonical upstream Evidence Semantic Input Authority in `hunter.evidence_semantic_inputs`; preserves observation-only `FundamentalEvidenceRecord`; provides immutable policy/output records, strict-known replay, correction/provenance, Evidence-Semantics-only consumption, and exact semantic lineage into assembled evidence; amends ADR 0021 and ADR 0025 only as stated |
