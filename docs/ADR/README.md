...
| [0036](0036-source-handling-design-implementation-contract.md) | Source Handling Design and Implementation Contract | Accepted | Binding realization of the ADR 0033 design and implementation contract: dedicated `SourceHandlingAuthorityService` sole-publisher ownership of the four record families (`FACT`/`POLICY`/`FIELD_CATEGORY_REGISTRY`/`AUTHORIZATION_RULE`) with bounded genesis bootstrap; `authorization_id`-bound signed `PublicationAuthorization`; fail-closed `UNKNOWN` semantics; callable `SourceHandlingAuthorityResolver`; transient-before-persistence Issue ingress ordering; strict-known correction/replay; additive migration and fail-closed rollback. Accepted under Issue #402. Runtime implementation is separately governed by Issue #407 and is not represented as complete. |
