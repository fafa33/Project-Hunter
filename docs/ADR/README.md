...
| [0033](0033-source-handling-classification-authority.md) | Source Handling Classification Authority | Proposed | Canonical Evidence Intelligence intake/document-lifecycle ownership of source-handling classification facts; immutable document-scoped classification events reusing accepted `VerifierType`; closed V1 handling vocabulary with fail-closed `unclassified`; span inheritance through the ADR 0031 `EvidenceContextSourceReference` without redefining it or changing `EvidenceSpan`; governed versioned retention-policy authority with deny-by-default hash/locator/coordinate/metadata categories; rejected-build audit, replay identities, legacy-version and typed-redaction rules; supplies the producer authority ADR 0031 left open without creating analytical authority |
