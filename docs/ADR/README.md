...
| [0033](0033-source-handling-classification-authority.md) | Source Handling Classification Authority | Proposed | Canonical Evidence Intelligence Source Handling Authority owning source-handling facts (intake/document-lifecycle producer) and retention policies (policy service); strictly separated `SourceHandlingFactSet`, `SourceHandlingClassificationEvent`, `RetentionPolicyVersion`, and derived `RetentionDecision`; five orthogonal closed V1 dimensions with restrictive join and no `retainable` fact; method/verifier admissibility with inadmissible caller assertion and release-authorized reductions; append-only streams with `BLOCKED` conflict and strict-known dual-cutoff selection; document-scoped scope with most-restrictive mixed-content resolution and read-only `EvidenceContextSourceReference`; deny-by-default categories, `READY`/`DEGRADED`/`BLOCKED` status, persistence rederivation, reference-safety-typed rejected-build audit, legacy-version and typed-content-state rules; supplies the producer authority ADR 0031 left open without changing `EvidenceSpan` or creating analytical authority |
