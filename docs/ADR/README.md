...
| [0028](0028-evidence-assembly-supporting-authorities.md) | Canonical Evidence Assembly Supporting Authorities | Proposed (revision 4) | Ownership, record family, persistence, versioning, correction, provenance, strict-known replay, conflict, and amendment-governance semantics for Methodology Contract — including a mandatory `value_capture_pathway_id` field — (owned by `hunter.valuation_methodology`), Evidence Shape Registry, Evidence Semantics, and `EvidenceSemanticsClassificationRuleset` (all owned by `hunter.evidence_assembly`); amends ADR 0025 (ownership silence, and an explicit `constituent_semantic_lineage` addition to the Assembled Fundamental Evidence record family's Lineage table) and ADR 0021 (additive `accepts_assembled_evidence` field on `ValuationMethodologySnapshot`'s field-list table, plus a `MethodologyEvidenceInputContract` cross-reference sentence); ADR 0022's Milestone-2 permitted values reaffirmed, unmodified |
