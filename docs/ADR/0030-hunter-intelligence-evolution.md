...
This ADR defines direction, authority boundaries, sequencing, interaction rules, and evaluation criteria. It does not claim that all described engines already exist or authorize immediate implementation without scoped design, independent review, and follow-up work.
...
The Prompt Intelligence Operating System is the priority consumer-facing subsystem of this layer. It must produce model-specific Prompt Packages from governed, model-independent context packages rather than embedding repository authority or evidence-selection policy inside ad hoc prompt strings.

A Prompt Package must eventually identify the task intent, role, evidence selected, evidence omitted and why, exact context ranges, model/provider target, token budget, output contract, provenance, and replay identity.
...
2. AI Intelligence receives a bounded task intent plus governed evidence references and produces versioned Prompt Packages, model executions, structured responses, and provenance records.
...
- Prompt Package — versioned role, task, instructions, context references, output schema, provider/model budget, and replay metadata.
...
7. Implement exact Replay and Provenance for Prompt Packages, requests, responses, corrections, and review decisions.
8. Implement Model and Provider Adapters for token counting, context and completion limits, structured output, rate limits, retries, and capability negotiation.
...
Metrics must be interpreted with context. They must not incentivize lower review quality, artificial prompt minimization, suppression of findings, or premature merging.
...
3. Reuse mature AI and engineering capabilities in Domain Intelligence only after personal validation shows that decisions are traceable, replayable, and trustworthy.
...
- The project accepts additional design discipline in exchange for model portability, auditability, reduced manual prompt hand-offs, lower avoidable token consumption, and reduced false approval or opaque decision risk.
