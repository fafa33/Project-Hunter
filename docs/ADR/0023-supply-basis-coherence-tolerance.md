...
That future identifier cannot itself be retrofitted onto records already persisted before it exists: `SupplyBasisSnapshot`s are append-only, and rewriting an already-accepted historical payload to add a field it never carried is prohibited by this repository's correction-lineage rules (a correction may only supersede, never mutate, its predecessor). Version-aware re-validation must therefore define an explicit, fixed legacy mapping for that gap, as part of the same future ADR: a record with no persisted tolerance-policy identifier is treated as carrying exactly the `0.0001` baseline this ADR authorizes — never defaulted to whatever the module constant or "current policy" then holds, which would silently recreate the same replay-corruption failure this rule exists to prevent. This mapping is permanent for every pre-existing record; it is not itself a migration step, and no already-persisted payload is rewritten to add the new field.
...
### Require the future versioned identifier on every record, including those already persisted before it exists

Rejected — a second independent review round on this ADR itself correctly identified that this would either reject every legacy `SupplyBasisSnapshot` outright when reconstructed through `_supply_from_payload` (since they carry no such field and records are append-only, so the field cannot be retrofitted onto them), or fall back to defaulting the missing field to "current policy," which silently recreates the exact replay-corruption failure this rule exists to prevent. The Decision section instead requires an explicit, fixed legacy mapping (absence of the identifier means the `0.0001` baseline this ADR authorizes, permanently, never "whatever is current") rather than a blanket requirement with no defined behavior for pre-existing data.
