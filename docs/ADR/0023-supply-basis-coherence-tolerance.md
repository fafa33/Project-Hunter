...
- Every `SupplyBasisSnapshot` accepted under the prior, strict, zero-tolerance check trivially satisfies the amended criterion (a tolerance of zero is a special case of a non-negative tolerance), so no replay or persisted-record compatibility issue exists.
