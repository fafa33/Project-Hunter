...
- **Runtime implementation: Phase A only, and not activated.** The pre-dispatch foundation is implemented under Issue #303 in `hunter.evidence_intelligence.model_adapter` and `model_adapter_persistence`: execution profile, attempt-time Source Handling re-resolution, per-category request-evidence decision, durable-before-send attempt, and single-use handoff, terminating at a no-network dispatch seam. No provider transport, provider SDK, endpoint, credential, live invocation, `ProviderResponseArtifact`, `ResponseValidator`, or routing exists, and none is authorized.
- **Conformance obligations CO-01 through CO-23: partially exercised, none discharged.** Phase A implements and regression-tests the pre-dispatch subset; every transport, response-capture, outcome, and reconciliation obligation remains an open gate for later phases.
