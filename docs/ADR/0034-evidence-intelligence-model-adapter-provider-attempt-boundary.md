...
- **Runtime implementation: Phases A and B, and not activated in production.** Phase A implemented the pre-dispatch foundation under Issue #303 in `hunter.evidence_intelligence.model_adapter` and `model_adapter_persistence`: execution profile, attempt-time Source Handling re-resolution, per-category request-evidence decision, durable-before-send attempt, and single-use handoff, terminating at a no-network dispatch seam, which remains present and unchanged. Phase B, implemented under Issue #313, adds `model_adapter_transport` with one provider transport for a single explicitly configured execution profile, a dispatch path that consumes the handoff atomically before sending, the append-only `ModelAttemptOutcomeRecord` family, and the governed `ProviderResponseArtifact` with its independent per-category durability and pre-construction credential capture gate. No `ResponseValidator`, semantic response validation, extraction or knowledge promotion, second provider, routing, ranking, fallback, hedging, or dynamic selection exists, and none is authorized. No provider credential, production endpoint, or live invocation is configured in the repository: the transport reaches a provider only when a deployment supplies both an endpoint for the profile's endpoint class and a credential at the non-durable transport boundary, and the deterministic test suite contacts no provider and opens no socket.
- **Conformance obligations CO-01 through CO-23: exercised across the implemented boundary; activation obligations remain the operator's.** Phase A regression-tests the pre-dispatch subset. Phase B adds deterministic regression coverage for the transport-isolation, single-use dispatch, append-only outcome, uncertain-delivery, retry-lineage, response-durability, structural-secret, response-authority-separation, routing-deferral, historical-reconstruction, and pre-dispatch-refusal obligations, each mutation-verified against the specific defect its regression names. The obligations that can only be discharged against a real deployment — production endpoint configuration, credential provisioning, and live provider idempotency and reconciliation behaviour — remain open and are not claimed as satisfied here.
...
Of those three activation preconditions, Phase B discharges the second and third for the implemented transport: the governed field-category registry expresses every request and response durable category the adapter writes, using the existing category vocabulary and requiring no Source Handling registry extension, and the transport's idempotency, correlation, and reconciliation semantics are classified on the execution profile with `UNAVAILABLE` and unknown both treated as retry-blocking. The first — the atomic snapshot-to-handoff guarantee — remains subject to the residual cross-substrate window Phase A recorded and did not close: the Source Handling authority store and the evidence database are separate substrates, re-resolution inside the write transaction narrows the exposure to that transaction, and closing it fully requires a governed cross-substrate commit primitive that does not yet exist. That window is unchanged by Phase B and is stated here rather than treated as discharged.
