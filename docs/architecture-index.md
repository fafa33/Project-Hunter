...
| [ADR 0033](ADR/0033-source-handling-classification-authority.md) | not applicable (owner-directed minimal decision scoped to the producer authority ADR 0031 left open; no governing ADPR was produced) | Canonical Evidence Intelligence intake/document-lifecycle ownership of source-handling classification facts, immutable document-scoped classification events, closed V1 handling vocabulary with fail-closed `unclassified`, governed versioned retention-policy authority and deny-by-default non-byte categories, rejected-build audit, replay identities, legacy-version and typed-redaction rules | Proposed; drafted to unblock PR #260, which remains draft; no runtime implementation is authorized; no analytical authority is created and no `ANALYTICAL_AUTHORITY_REGISTRY` entry is required |
