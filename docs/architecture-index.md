...
| [ADR 0033](ADR/0033-source-handling-classification-authority.md) | not applicable (owner-directed minimal decision scoped to the authority ADR 0031 left open; no governing ADPR was produced) | Exclusive Evidence Intelligence consumer-side ownership of authoritative source-handling facts and governed retention-policy authority, with binding safety, historical/replay, and persistence invariants; all representation, lifecycle, identity, selection, category, audit, decoding, and test mechanics deferred to a separate design and implementation contract | Proposed; minimized so the ADR binds authority and invariants rather than runtime specification; drafted to unblock PR #260, which remains draft; no runtime implementation is authorized; no analytical authority is created and no `ANALYTICAL_AUTHORITY_REGISTRY` entry is required |
