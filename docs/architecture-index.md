...
| [ADPR-0005](architecture-records/ADPR-0005-evidence-assembly-supporting-authorities.md) | Canonical Evidence Assembly Supporting Authorities | READY_FOR_REVIEW | None material — revision 4 resolves all four revision-1 open questions plus three independent-review rounds' findings (round 1: five findings; round 2: four findings; round 3: governing-snapshot strict-known coordinate defect, missing `value_capture_pathway_id` field, missing `constituent_semantic_lineage` persistence) against Draft PR #192; a fourth independent review round is pending | not yet assigned |
...
| [ADR 0028](ADR/0028-evidence-assembly-supporting-authorities.md) | [ADPR-0005](architecture-records/ADPR-0005-evidence-assembly-supporting-authorities.md) | Ownership, persistence, versioning, correction, provenance, strict-known replay, conflict, and amendment-governance semantics for Methodology Contract (owned by `hunter.valuation_methodology`; identity is `(contract_id, contract_version)`, its actual pre-existing fields, now including a mandatory `value_capture_pathway_id`), Evidence Shape Registry, Evidence Semantics, and `EvidenceSemanticsClassificationRuleset` (all owned by `hunter.evidence_assembly`); amendments to ADR 0025 (ownership silence, plus an explicit `constituent_semantic_lineage` addition to the Assembled Fundamental Evidence Lineage table) and to ADR 0021's field-list table (additive `accepts_assembled_evidence` field on `ValuationMethodologySnapshot`, plus a `MethodologyEvidenceInputContract` cross-reference sentence); ADR 0022 reaffirmed, unmodified | Proposed, revision 4; drafted under Issue #191; resolved five round-1, four round-2, and three round-3 independent-review `CHANGES REQUIRED` findings; pending a fourth independent review round and acceptance; no implementation authorized |
...
| Issue #191 | [ADPR-0005](architecture-records/ADPR-0005-evidence-assembly-supporting-authorities.md) | [ADR 0028](ADR/0028-evidence-assembly-supporting-authorities.md) (Proposed) | [PR #192](https://github.com/fafa33/Project-Hunter/pull/192) (Draft); implementation not yet created | Architecture preparation only; prerequisite for unblocking Issue #190; revision 4 resolves five round-1, four round-2, and three round-3 independent-review `CHANGES REQUIRED` findings; awaiting a fourth independent review round and acceptance before any implementation may begin |
