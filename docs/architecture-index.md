...
| [ADPR-0009](architecture-records/ADPR-0009-evidence-intelligence-model-adapter.md) | Evidence Intelligence Model Adapter and Provider Attempt Boundary | READY_FOR_REVIEW | None material — the independent architecture audit is complete and returned `READY_FOR_ADR` with no blocking finding (merged in PR #290 at `09603c7b0ee3076f902190a0c1c1d223f9d71d8b`), and ADR 0034, drafted under Issue #299, is Accepted under Issue #301 as binding architecture with runtime implementation not started. The preparation record's own lifecycle transition beyond `READY_FOR_REVIEW` remains a separate owner decision and is not authorized by the ADR-drafting lifecycle | repository owner-directed architecture work |
...
| Issue #301 | [ADPR-0009](architecture-records/ADPR-0009-evidence-intelligence-model-adapter.md) | [ADR 0034](ADR/0034-evidence-intelligence-model-adapter-provider-attempt-boundary.md) (Accepted) | ADR acceptance contribution; implementation not yet created | Architecture: Accepted — ADR 0034 is binding. Implementation: not started and not activated; no ADR 0034-conformant Model Adapter runtime, provider transport, credential, `ResponseValidator`, or routing exists, and CO-01 through CO-23 remain unsatisfied future gates. The legacy provider path remains outside this boundary and is not Model Adapter implementation. Implementation requires a separate authorized lifecycle |
...
| AI Context and Prompt Intelligence | [ADPR-0009](architecture-records/ADPR-0009-evidence-intelligence-model-adapter.md) | [ADR 0031](ADR/0031-ai-context-prompt-intelligence-foundation.md) (Accepted); [ADR 0032](ADR/0032-project-agnostic-prompt-intelligence-core.md) (Accepted); [ADR 0033](ADR/0033-source-handling-classification-authority.md) (Accepted); [ADR 0034](ADR/0034-evidence-intelligence-model-adapter-provider-attempt-boundary.md) (Accepted) | ADR 0031 remains authoritative for Hunter's concrete pre-model contracts and its provider-free runtime exists in current source. ADR 0032 establishes the accepted project-neutral boundary and evidence-gated admission rule. ADR 0033 establishes the accepted consumer-side Source Handling Authority and its mechanics are integrated by later governed work. ADPR-0009 prepares the separately deferred Model Adapter/provider-attempt boundary and its independent audit returned `READY_FOR_ADR`. ADR 0034 formalizes that boundary and is Accepted under Issue #301 as binding architecture; its runtime implementation is not started and not activated, and neither preparation nor acceptance authorizes runtime invocation, provider activation, or credentials. |
