...
- **Probability of Outperformance** — likelihood the target structurally outperforms its evidence-defined peer/reference set over the assessed horizon band.
- **Probability of Underperformance** — the complementary structural read, not derived by simple subtraction from outperformance when peer-set membership itself is uncertain; disclosed with its own evidence trail.
...
- **Probability of Failure** — likelihood of reaching an evidence-defined structural failure condition (sustained deterioration, invalidation, abandonment signals).
...
- **Probability of Risk Escalation**, **Probability of Structural Deterioration** — the mirrored downside category probabilities, defined symmetrically to their upside counterparts.
- **Probability of 2x / 5x / 10x** — frequency-derived probabilities from the historical analog set of structurally similar past cases reaching each multiple, always disclosed with analog sample size; never a price simulation.
- **Probability of Permanent Loss** — frequency-derived probability of historical analogs that were invalidated, abandoned, or structurally impaired beyond recovery.
- **Expected Reward Band** — a magnitude band (not a number) from the dispersion of historical analog upside outcomes.
- **Expected Risk Band** — a magnitude band from the dispersion of historical analog downside outcomes.
...
- The Historical Analog Module reuses the exact point-in-time analog-matching discipline established by the Opportunity Timing Engine's Historical Similarity Module: only evidence available as of each analog's own `as_of` is used to classify that analog, and only evidence available as of the current `as_of` is used to match against it.
...
- The engine never queries Historical Validation for anything other than persisted, leakage-checked, point-in-time case records — it has no separate access to raw historical market data.
...
- The Opportunity Timing Engine's Scenario Analysis (Bull/Base/Bear analog buckets) is folded in as a structured, already-computed evidence update rather than being recomputed by the Historical Analog Module a second time — the two engines share the same underlying analog-matching discipline (Section 11) but do not duplicate each other's query.
