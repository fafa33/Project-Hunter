...
- Reviewers: three independent hostile architecture review rounds against Draft PR #192 — round 1 (five `CHANGES REQUIRED` findings, resolved in revision 2), round 2 (four `CHANGES REQUIRED` findings, resolved in revision 3), round 3 (three `CHANGES REQUIRED` findings, resolved in revision 4 below); a fourth independent review round has not yet occurred
- Created: 2026-08-04
- Revised: 2026-08-04 (revision 2, resolving round-1 findings; revision 3, resolving round-2 findings; revision 4, resolving round-3 findings)
...
**Revision 2 resolved revision 1's four open questions. Revision 3 resolved round 2's four contradictions.** A third independent hostile architecture review round then found three further defects — not new design questions, but places where revision 3's corrected text was itself still executing the wrong query, still missing a field its own prose depended on, or still not persisting information its own validation logic already resolved and silently discarded. **Revision 4 (this revision) eliminates all three**, without redesigning the architecture round 3 otherwise confirmed:

- **Methodology Contract Authority — governing-snapshot coordinate defect.** Round 2's Finding 1 was about executability (revision 2's call used parameters `strict_known_methodology` does not accept); round 3's Finding 1 is about correctness of an executable call: revision 3's fix passed the *referenced snapshot's own* `effective_at` as the query's `effective_as_of` bound, which by `_strict_known`'s own `effective_at <= effective_as_of` filter can never observe a successor whose `effective_at` is later — permanently blind to exactly the "later-effective successor disables assembled evidence" scenario the finding names. Revision 4 substitutes the assembly's own decision coordinates (`accounting_window_end` as `effective_as_of`, `replay_cutoff` as `known_by`) for the query, and additionally identifies and authorizes the one Protocol-signature gap (`strict_known_contract` has never carried an `effective_as_of` parameter, unlike every other strict-known selector in this codebase) that made the wrong substitution the only one revision 3's algorithm shape could reach.
- **Methodology Contract Authority — `value_capture_pathway_id` field gap.** Revision 3's `contract_id` hash formula and its "Supporting authority ownership" prose both referenced `value_capture_pathway_id` as part of the contract's identity; the dataclass itself never gained that field. Revision 4 adds it as a mandatory field, threads it through identity, persistence, correction, and conflict rules, and authorizes the one additive `_validate_methodology_contract` check needed to actually verify it against an assembly request — closing the gap between prose and dataclass rather than removing pathway from scope, since ADR 0025 (Accepted) independently requires pathway-specific eligibility (Assembly precondition 3; "Methodology contract" continuity requirements; the already-accepted `AssembledFundamentalEvidenceRecord` Scope table).
- **Assembled Fundamental Evidence record family — missing semantic provenance.** `_validate_authoritative_semantics` already resolves, per constituent, the exact `AuthoritativeEvidenceSemantics` classification consulted, but `AssembledFundamentalEvidenceRecord` never persisted that resolution — only native-evidence lineage, never semantic-classification lineage. Revision 4 adds a new required field, `constituent_semantic_lineage`, as an explicit, acknowledged amendment to ADR 0025's own record-family field list (not claimed to leave that family "unchanged"), with full replay, correction, and hash-participation rules, and authorizes the additional Protocol/return-value changes needed to actually populate it.
...
| E-019 | Independent hostile architecture review of Draft PR #192, round 3 | External review input (three `CHANGES REQUIRED` findings; see "Decision History") | Identified: (1) the governing-snapshot check revision 3 authored is executable but wrong — it queries `strict_known_methodology` at the referenced snapshot's own `effective_at`, which by construction can never observe a later-effective successor; (2) `value_capture_pathway_id` is referenced in the `contract_id` hash formula and in prose but was never added as an actual `MethodologyEvidenceInputContract` field; (3) `AssembledFundamentalEvidenceRecord` has no field capable of reconstructing the exact evidence-semantics classification and ruleset version each constituent was validated against, insufficient for ADR 0025's own already-accepted exact-replay requirement. | Authoritative for this revision's scope; same recorded-artifact caveat as rounds 1-2 (E-014/E-018, above). | Directly drives every design correction in revision 4. |
| E-020 | `src/hunter/valuation_methodology/service.py:214-231` (`_strict_known`), re-verified in this revision | Direct code read | `_strict_known` filters candidates by `effective_at <= effective_as_of`; passing a fixed, already-known snapshot's own `effective_at` as that bound permanently excludes any candidate whose own `effective_at` is later, regardless of `known_by`. | Reproducible. | Directly resolves round-3 Finding 1 — proves revision 3's coordinate choice, not just its API executability, was the defect; confirms the fix must supply the assembly's own `effective_as_of`, never the referenced snapshot's. |
| E-021 | `src/hunter/evidence_assembly/models.py:99-125` (`MethodologyEvidenceInputContract`), re-verified in this revision | Direct code read | The dataclass's full field list carries no `value_capture_pathway_id` field, confirming revision 3's `contract_id` hash formula and ownership prose referenced a value the dataclass could not actually carry. | Reproducible. | Directly resolves round-3 Finding 2 — confirms the gap is real, not a documentation-only nitpick, since a hash formula referencing a nonexistent field cannot be implemented as written. |
| E-022 | `src/hunter/evidence_assembly/service.py:271-310` (`_validate_authoritative_semantics`) and `src/hunter/evidence_assembly/models.py:171-295` (`AssembledFundamentalEvidenceRecord`), re-verified in this revision | Direct code read | `_validate_authoritative_semantics` already resolves a `semantics: AuthoritativeEvidenceSemantics` object per constituent (via `strict_known_semantics`) but returns `None`, discarding it; `AssembledFundamentalEvidenceRecord`'s full field list (Identity/Scope/Lineage/Temporal/Quality/Correction) contains no field referencing any `AuthoritativeEvidenceSemantics` or `EvidenceSemanticsClassificationRuleset` identity at all. | Reproducible. | Directly resolves round-3 Finding 3 — confirms the information needed for exact semantic-lineage persistence is already computed at assembly time and only needs to be captured and persisted, not newly derived. |
...
- **Risk R-006 (new in revision 4)**: five narrowly bounded, explicitly enumerated Protocol/call-site additions to already-accepted, currently production-unreachable `hunter.evidence_assembly` code are now required before implementation can proceed (ADR 0028 §"Exact, bounded additions to already-accepted `hunter.evidence_assembly` code"): an `effective_as_of` parameter on `MethodologyContractAuthority.strict_known_contract`; one pathway-equality check in `_validate_methodology_contract`; a `strict_known_ruleset` method on the `EvidenceSemanticsAuthority` Protocol; a return-value change to `_validate_authoritative_semantics`; nested-datetime (de)serialization in `AssembledEvidenceRepository`. None is a behavior change for any currently-reachable caller (none exists), but all five are real implementation cost a future issue must budget for explicitly, not treat as already covered by "additive fields."
- **Risk R-007 (new in revision 4)**: `AssembledFundamentalEvidenceRecord`'s new `constituent_semantic_lineage` field is not backward-compatible with any hypothetical pre-existing record of this family — since no production implementation of any of the three authorities exists yet (E-003/E-004), no real `AssembledFundamentalEvidenceRecord` has ever been persisted, so this has no migration consequence today, but a future implementation issue must not treat the field as optional or backfill it with a placeholder; every constructed record must carry a real, resolved lineage tuple from the moment construction is implemented.
...
Round 3's three further findings are likewise resolved in revision 4, not carried forward:

- **Finding 1 (resolved)**: the governing-snapshot check now queries `strict_known_methodology` at the assembly's own `(effective_as_of, known_by)` decision coordinates (`accounting_window_end`, `replay_cutoff`) rather than the referenced snapshot's own `effective_at` (E-020); the required `effective_as_of` Protocol-signature addition this fix depends on is explicitly authorized, not left implicit (ADR 0028 §"Exact, bounded additions").
- **Finding 2 (resolved)**: `value_capture_pathway_id` is now an actual, mandatory `MethodologyEvidenceInputContract` field (E-021), with full identity, persistence, validation, correction, and conflict rules — not only a hash-formula reference.
- **Finding 3 (resolved)**: `AssembledFundamentalEvidenceRecord` now carries `constituent_semantic_lineage` (E-022), an explicit, acknowledged amendment to ADR 0025's own record-family field list, with full replay, correction, and hash-participation rules.
...
- No accepted ADR is superseded, weakened, or contradicted. ADR 0028 amends both ADR 0025 **and** ADR 0021 — the ADR 0021 amendment was misattributed to ADR 0022 in revision 2 (a self-contradiction independent review's second round correctly caught, since the actual edited text is an ADR 0021 table) and corrected to ADR 0021 in revision 3. ADR 0022 itself — the Milestone-2 permitted-value invariants — is reaffirmed, wholly unmodified.
- **New in revision 4**: the ADR 0021 amendment now additionally adds a cross-reference sentence for the `MethodologyEvidenceInputContract` family itself (mirroring `AssembledFundamentalEvidenceRecord`'s existing treatment in the same table), and the ADR 0025 amendment now additionally, explicitly amends the "Assembled Fundamental Evidence record family" → "Lineage" table to add `constituent_semantic_lineage` — stated as a real, acknowledged change to that record family, not absorbed into "unchanged" language elsewhere in ADR 0028.
...
| Comparative fairness | GOOD | The adopted design is compared against every alternative a reviewer could plausibly have preferred, on the same criteria, including two new alternatives this revision rejects (re-deriving semantic lineage at replay time; a narrower lineage entry). | None |
...
| Persistence and replay | GOOD | Full envelope, uniqueness constraint, and strict-known selection algorithm specified per authority, each proven unambiguous by construction; the governing-snapshot check now queries at the correct strict-known coordinates (E-020), and semantic-lineage replay is now exact rather than re-derived (E-022). | None |
| Evidence and provenance | GOOD | Evidence Semantics' authoring model is fully specified (deterministic ruleset, manual exceptions prohibited); `AssembledFundamentalEvidenceRecord` now durably records exactly which semantic classification and ruleset version validated each constituent, closing the round-3 Finding-3 gap. | None |
| Implementation impact | ACCEPTABLE | Bounded, real implementation cost remains (Risk R-003 test-fixture updates; Risk R-005 first-content publication; Risk R-006, new in revision 4, five explicit Protocol/call-site additions; Risk R-007, new in revision 4, no backward-compatibility burden since no record of this family has ever been persisted) — correctly stated as cost, not architectural gap. | None |
| Governance compatibility | GOOD | No accepted ADR is superseded, weakened, or contradicted; required amendments (ADR 0025 — now including an explicit Lineage-table addition — and ADR 0021 — now including a `MethodologyEvidenceInputContract` cross-reference sentence) are narrow, explicitly scoped, and attributed to their real canonical owner. | None |
| Traceability | GOOD | Issue #191, this ADPR, ADR 0028, and the independent review findings driving this revision are cross-linked. | Fourth independent review round pending |
...
- Decisions the ADR must fix: all fixed in ADR 0028 revision 4; none remain for independent review to send back as "still open."
...
Advance the adopted design (ADR 0028, revision 4) to a fourth round of independent architecture review. This session's author has not approved its own work — ADR 0028 remains Status `Proposed` and this record remains `READY_FOR_REVIEW`, not `APPROVED`, pending that review.
...
| 2026-08-04 | READY_FOR_REVIEW | Third independent hostile architecture review round of Draft PR #192 (revision 3): three `CHANGES REQUIRED` findings — (1) the governing-snapshot check revision 3 authored queries `strict_known_methodology` at the referenced snapshot's own `effective_at`, which can never observe a later-effective successor that disables assembled evidence; (2) `value_capture_pathway_id` is referenced in the `contract_id` hash formula and in prose but was never added as an actual `MethodologyEvidenceInputContract` field; (3) `AssembledFundamentalEvidenceRecord` has no field capable of reconstructing the exact evidence-semantics classification and ruleset version each constituent was validated against. | Independent review, round 3 (see the governing session's report for the same recorded-artifact caveat noted for rounds 1-2) |
| 2026-08-04 | READY_FOR_REVIEW | Revision 4: all three round-3 findings resolved without redesigning the architecture round 3 otherwise confirmed — governing-snapshot check now queries at the assembly's own `(effective_as_of, known_by)` decision coordinates via an explicitly authorized `strict_known_contract` Protocol addition (resolves 1); `value_capture_pathway_id` added as a mandatory, hashed, validated `MethodologyEvidenceInputContract` field with full correction/conflict rules, plus the additive `_validate_methodology_contract` check needed to actually enforce it (resolves 2); `constituent_semantic_lineage` added to `AssembledFundamentalEvidenceRecord` as an explicit ADR 0025 amendment, with full replay/correction/hash rules and the additive Protocol/service changes needed to populate it (resolves 3). New in this revision: a consolidated "Exact, bounded additions to already-accepted `hunter.evidence_assembly` code" inventory in ADR 0028, listing every required Protocol/call-site change explicitly rather than leaving any implicit. | Claude |
...
This record is `READY_FOR_REVIEW`, not `APPROVED`. It is a working preparation artifact until independent architecture review completes. This is revision 4, corrected in place across three rounds of independent review findings (against revision 1, revision 2, then revision 3) — the "Decision History" table above is the permanent record of what changed and why, consistent with `docs/ARCHITECTURE_DECISION_PREPARATION_GUIDE.md`'s allowance for in-place correction before approval. After approval, substantive corrections require a new ADPR that explicitly supersedes this one, consistent with ADPR-0004's precedent.
