...
def rank_opportunities(
    ...
    *,
    ...
    if sort == "opportunity":
        return tuple(sorted(assessments, key=lambda item: (-item.opportunity_score, -item.conviction_score, item.project_id)))
    if sort == "conviction":
        return tuple(sorted(assessments, key=lambda item: (-item.conviction_score, -item.opportunity_score, item.project_id)))
