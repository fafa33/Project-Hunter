...
def assess_divergence(
    ...
    if avg.get("social", 0.0) >= rules.get("social_high", 0.75) and max(
        avg.get("protocol", 0.0), avg.get("on-chain", 0.0), avg.get("developer", 0.0)
    ) < rules.get("fundamental_low", 0.45):
        divergences.append("social_excitement_without_fundamental_confirmation")
    if min(avg.get("protocol", 1.0), avg.get("developer", 1.0)) >= rules.get("fundamental_high", 0.65) and avg.get(
        "social", 0.0
    ) < rules.get("attention_low", 0.4):
        divergences.append("fundamentals_without_market_attention")
    if avg.get("whale", 0.0) >= rules.get("whale_high", 0.7) and avg.get("on-chain", 0.0) < rules.get(
        "fundamental_low", 0.45
    ):
        divergences.append("whale_accumulation_without_broad_adoption")
    if avg.get("narrative", 0.0) >= rules.get("narrative_high", 0.7) and avg.get("developer", 0.0) < rules.get(
        "fundamental_low", 0.45
    ):
        divergences.append("narrative_acceleration_without_developer_activity")
    if avg.get("on-chain", 0.0) >= rules.get("fundamental_high", 0.65) and avg.get("macro", 1.0) < rules.get(
        "macro_low", 0.4
    ):
        divergences.append("adoption_growth_under_macro_headwinds")
    if (
        avg.get("social", 0.0) >= rules.get("social_saturation", 0.85)
        and min(avg.get("protocol", 1.0), avg.get("developer", 1.0)) >= 0.55
    ):
    ...
    return DivergenceState(
        tuple(divergences), severity, f"{len(divergences)} deterministic divergence patterns detected."
    )
