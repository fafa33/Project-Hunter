...
from hunter.committee.engine import (
    ...
    def evaluate_cycle(
        ...
        if not inputs:
            raise CommitteeAuthorityError(
                "committee cycle requires at least one candidate"
            )
...
def _validate_inputs(inputs: tuple[CommitteeInputSet, ...]) -> None:
    ...
    if any(item.effective_at != effective_at for item in inputs):
        raise CommitteeAuthorityError(
            "all candidates in a cycle must share effective_at"
        )
    for item in inputs:
...
def _validate_persisted_input(
    ...
    if not record_id:
        raise CommitteeAuthorityError(
            "all committee inputs must reference persisted record IDs"
        )
...
def _validate_assessment_input(
    ...
    if not assessment_id:
        raise CommitteeAuthorityError(
            "all derived committee inputs must reference persisted assessment IDs"
        )

    effective_at = getattr(assessment, "effective_at", None)
    if not isinstance(effective_at, datetime):
        raise CommitteeAuthorityError(
            "derived committee inputs must define effective_at"
        )
    if effective_at > item.effective_at:
        raise CommitteeAuthorityError(
            "future-effective assessment cannot enter committee evaluation"
        )
    ...
    if isinstance(recorded_at, datetime) and recorded_at > item.effective_at:
        raise CommitteeAuthorityError(
            "future-known assessment cannot enter committee evaluation"
        )
...
def _champion_from_ranked(
    ...
    if selected is not None and selected != winner.project_id:
        raise CommitteeAuthorityError(
            "engine champion does not match authoritative ranking"
        )
    return replace(
        original_champion,
        ...
        lead_margin=max(
            0.0,
            winner.committee_confidence
            - (runner_up.committee_confidence if runner_up else 0.0),
        ),
    )
