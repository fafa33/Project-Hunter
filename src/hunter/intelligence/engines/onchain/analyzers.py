...
class OnchainAnalyzer:
    ...
    def analyze(self, dataset: OnchainDataset) -> OnchainAnalysis:
        ...
        strengths = tuple(sorted(indicator.name for indicator in indicators if indicator.direction == "positive" and indicator.value >= 0.6))
        risks = tuple(sorted(indicator.name for indicator in indicators if indicator.direction == "negative" and indicator.value >= 0.45))
        ...
        return OnchainAnalysis(
            ...
            concentration="concentrated" if by_name.get("holder_concentration", _empty()).value >= self.configuration.holder_concentration_threshold else "distributed",
            decentralization="improving" if by_name.get("supply_distribution_quality", _empty()).value >= 0.55 else "weak",
            ...
            metadata={"chains": str(len({record.chain for record in dataset.records})), "assets": str(len({record.asset for record in dataset.records}))},
        )
