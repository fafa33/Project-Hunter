...
class OnchainIndicatorCalculator:
    ...
    def returning_address_ratio(self, dataset: OnchainDataset) -> OnchainIndicator:
        ...
        return _indicator("returning_address_ratio", _ratio(latest.returning_addresses or 0, latest.active_addresses), "positive", "Returning address ratio.")

    def address_retention(self, dataset: OnchainDataset) -> OnchainIndicator:
        latest = dataset.addresses[-1] if dataset.addresses else None
        if latest is None or not latest.active_addresses:
            return _missing("address_retention", "addresses")
        return _indicator("address_retention", _ratio(latest.retained_addresses or 0, latest.active_addresses), "positive", "Retained address ratio.")

    def adjusted_transaction_growth(self, dataset: OnchainDataset) -> OnchainIndicator:
        values = [item.adjusted_transaction_count for item in dataset.transactions if item.adjusted_transaction_count is not None]
    ...
    def exchange_netflow(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _signed_indicator("exchange_netflow", self._flows.exchange_netflow(dataset), "Exchange outflow minus inflow balance.")

    def bridge_netflow(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _signed_indicator("bridge_netflow", self._flows.bridge_netflow(dataset), "Bridge inflow minus outflow balance.")
    ...
    def capital_retention(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("capital_retention", self._flows.capital_retention(dataset), "positive", "Capital retained after inflow.")
    ...
    def long_term_holder_growth(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("long_term_holder_growth", self._holders.long_term_holder_growth(dataset), "positive", "Long-term holder growth.")

    def holder_concentration(self, dataset: OnchainDataset) -> OnchainIndicator:
        value = self._holders.concentration(dataset)
        return _indicator("holder_concentration", value, "negative" if value >= self.configuration.holder_concentration_threshold else "positive", "Top-holder concentration.")

    def supply_distribution_quality(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("supply_distribution_quality", self._holders.supply_distribution_quality(dataset), "positive", "Supply distribution quality.")

    def accumulation_breadth(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("accumulation_breadth", self._holders.accumulation_breadth(dataset), "positive", "Accumulation breadth.")

    def distribution_breadth(self, dataset: OnchainDataset) -> OnchainIndicator:
        value = self._holders.distribution_breadth(dataset)
        return _indicator("distribution_breadth", value, "negative" if value >= 0.35 else "positive", "Distribution breadth.")

    def contract_activity_growth(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("contract_activity_growth", self._contracts.active_contract_growth(dataset), "positive", "Active contract growth.")

    def contract_diversity(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("contract_diversity", self._contracts.contract_diversity(dataset), "positive", "Contract and chain diversity.")
    ...
    def application_concentration(self, dataset: OnchainDataset) -> OnchainIndicator:
        ...
        return _indicator("application_concentration", value, "negative" if value >= self.configuration.application_concentration_threshold else "positive", "Application activity concentration.")

    def token_velocity(self, dataset: OnchainDataset) -> OnchainIndicator:
        latest_supply = dataset.supply[-1].circulating_supply if dataset.supply and dataset.supply[-1].circulating_supply else None
        latest_volume = dataset.transactions[-1].adjusted_volume if dataset.transactions and dataset.transactions[-1].adjusted_volume else None
        if not latest_supply or latest_volume is None:
            return _missing("token_velocity", "supply_or_volume")
        return _indicator("token_velocity", min(latest_volume / latest_supply, 1.0), "neutral", "Adjusted volume relative to circulating supply.")
    ...
    def network_participation(self, dataset: OnchainDataset) -> OnchainIndicator:
        ...
        return _indicator("network_participation", mean((address, contract, governance)), "positive", "Network participation breadth.")

    def validator_concentration(self, dataset: OnchainDataset) -> OnchainIndicator:
        values = [item.top_validator_share or item.staker_concentration for item in dataset.validators if (item.top_validator_share or item.staker_concentration) is not None]
        value = mean(values) if values else 0.0
        return _indicator("validator_concentration", value, "negative" if value >= 0.45 else "positive", "Validator or staker concentration.")
    ...
    def governance_participation(self, dataset: OnchainDataset) -> OnchainIndicator:
        ...
        return _indicator("governance_participation", mean(values) if values else 0.0, "positive", "On-chain governance participation.")

    def circular_flow_risk(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("circular_flow_risk", self._flows.circular_flow_risk(dataset), "negative", "Circular flow risk.")

    def wash_activity_risk(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("wash_activity_risk", self._anomalies.wash_activity_risk(dataset), "negative", "Wash activity risk.")
    ...
    def bot_activity_risk(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("bot_activity_risk", self._anomalies.bot_activity_risk(dataset), "negative", "Bot activity risk.")

    def bridge_pass_through_risk(self, dataset: OnchainDataset) -> OnchainIndicator:
        return _indicator("bridge_pass_through_risk", self._flows.bridge_pass_through_risk(dataset), "negative", "Bridge pass-through risk.")
    ...
    def onchain_acceleration(self, dataset: OnchainDataset) -> OnchainIndicator:
        value = mean((self.active_address_momentum(dataset).value, self.adjusted_transaction_growth(dataset).value, self.net_capital_flow(dataset).value, self.holder_growth(dataset).value, self.contract_activity_growth(dataset).value))
    ...
    def onchain_deterioration(self, dataset: OnchainDataset) -> OnchainIndicator:
        value = mean((self.churn(dataset).value, self.holder_concentration(dataset).value, self.anomaly_severity(dataset).value, 1.0 - self.net_capital_flow(dataset).value))
        return _indicator("onchain_deterioration", value, "negative" if value >= 0.5 else "positive", "Combined on-chain deterioration.")
...
def _indicator(name: str, value: float, direction: str, description: str) -> OnchainIndicator:
    return OnchainIndicator(name=name, value=round(_clamp(value), 4), direction=direction, confidence=0.85, description=description)


def _missing(name: str, evidence_name: str) -> OnchainIndicator:
    return OnchainIndicator(name=name, value=0.0, direction="unknown", confidence=0.0, description=f"Missing {evidence_name} evidence.", missing_evidence=(evidence_name,))
