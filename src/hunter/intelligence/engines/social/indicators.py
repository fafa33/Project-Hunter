...
class SocialIndicatorCalculator:
    ...
    def mention_momentum(self, dataset: SocialDataset) -> SocialIndicator:
        return _indicator(
            "mention_momentum",
            _recent_share([post.timestamp for post in dataset.posts]),
            "positive",
            "Recent mention share.",
        )
    ...
    def unique_author_growth(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        recent = {
            ...
            if post.timestamp >= latest - timedelta(days=self.configuration.freshness_days)
        }
        all_authors = {post.author_id for post in dataset.posts}
        return _indicator(
            "unique_author_growth", _ratio(len(recent), len(all_authors)), "positive", "Recent unique author share."
        )
    ...
    def engagement_quality(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            "engagement_quality",
            mean(item.quality for item in dataset.engagements),
            "positive",
    ...
    def discussion_quality(self, dataset: SocialDataset) -> SocialIndicator:
        if dataset.conversations:
            return _indicator(
                "discussion_quality",
                mean(item.quality for item in dataset.conversations),
                "positive",
        ...
        return _indicator(
            "discussion_quality", _ratio(original, len(dataset.posts)), "positive", "Original discussion share."
        )
    ...
    def community_retention(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            "community_retention",
            _ratio(latest.retained_members, latest.active_members),
            "positive",
            "Retained active community members.",
        )

    def platform_diversity(self, dataset: SocialDataset) -> SocialIndicator:
        platforms = {post.platform for post in dataset.posts} | {
            community.platform for community in dataset.communities
        }
    ...
    def author_diversity(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            "author_diversity",
            _ratio(len(unique_authors), len(dataset.posts)),
            "positive",
            "Unique authors relative to posts.",
        )
    ...
    def influence_quality(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            "influence_quality",
            mean(self._influence.score_author(author) for author in dataset.authors),
            "positive",
            "Influence quality beyond follower count.",
        )
    ...
    def influence_concentration(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            ...
            1.0 - concentration,
            "negative" if concentration >= 0.55 else "positive",
            "Lower influence concentration is healthier.",
        )
    ...
    def sentiment_dispersion(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            ...
            dispersion,
            "negative" if dispersion >= 0.35 else "positive",
            "Sentiment dispersion and disagreement.",
        )
    ...
    def attention_persistence(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            ...
            min(depth / self.configuration.minimum_historical_depth_days, 1.0),
            "positive",
            "Historical depth of attention.",
        )

    def attention_acceleration(self, dataset: SocialDataset) -> SocialIndicator:
        return _indicator(
            "attention_acceleration", self.mention_momentum(dataset).value, "positive", "Attention acceleration."
        )
    ...
    def attention_saturation(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            ...
            saturation,
            "negative" if saturation >= self.configuration.saturation_threshold else "positive",
            "High attention density can indicate saturation.",
        )
    ...
    def organic_attention_ratio(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        organic = sum(
            1 for post in dataset.posts if not post.promotional and not post.giveaway and not post.coordinated
        )
        return _indicator(
            "organic_attention_ratio", _ratio(organic, len(dataset.posts)), "positive", "Organic post share."
        )

    def promotional_activity_ratio(self, dataset: SocialDataset) -> SocialIndicator:
        return _indicator(
            "promotional_activity_ratio",
            self._manipulation.promotion_risk(dataset),
            "negative",
    ...
    def coordination_risk(self, dataset: SocialDataset) -> SocialIndicator:
        return _indicator(
            "coordination_risk", self._manipulation.coordination_risk(dataset), "negative", "Coordinated campaign risk."
        )
    ...
    def manipulation_risk(self, dataset: SocialDataset) -> SocialIndicator:
        ...
        return _indicator(
            "manipulation_risk",
            max(assessment.bot_risk, assessment.spam_risk, assessment.promotion_risk, assessment.coordination_risk),
            "negative",
    ...
    def narrative_alignment(self, dataset: SocialDataset) -> SocialIndicator:
        return _indicator(
            "narrative_alignment",
            dataset.narrative_alignment,
            "positive",
            "Alignment with existing Narrative Intelligence.",
        )

    def social_acceleration(self, dataset: SocialDataset) -> SocialIndicator:
        value = mean(
            (
                self.mention_momentum(dataset).value,
                self.unique_author_growth(dataset).value,
                self.engagement_momentum(dataset).value,
            )
        )
    ...
    def social_deterioration(self, dataset: SocialDataset) -> SocialIndicator:
        value = 1.0 - mean(
            (
                self.social_acceleration(dataset).value,
                self.community_retention(dataset).value,
                self.organic_attention_ratio(dataset).value,
            )
        )
        return _indicator(
            "social_deterioration",
            value,
            "negative" if value >= 0.55 else "neutral",
            "Weakening social quality and momentum.",
        )
