...
def snapshot_value(snapshots: tuple[SnapshotRecord, ...], key: str) -> float:
    return average(tuple(numeric(record.payload[key]) for record in snapshots if key in record.payload))
