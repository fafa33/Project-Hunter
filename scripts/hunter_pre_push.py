...
ZERO_SHA = "0" * 40
PREFLIGHT_COMMAND = ("python", "scripts/hunter_pr_preflight.py", "--mode", "normal")
...
def _parse_updates(lines: Iterable[str]) -> list[tuple[str, str]]:
...
def _require_exact_head(updates: list[tuple[str, str]], head_sha: str) -> None:
    branch_updates = [(ref, sha) for ref, sha in updates if ref.startswith("refs/heads/")]
    if not branch_updates:
        return
    mismatched = [(ref, sha) for ref, sha in branch_updates if sha != head_sha]
    if mismatched:
        refs = ", ".join(ref for ref, _sha in mismatched)
        raise RuntimeError(
...
def enforce_pre_push(lines: Iterable[str]) -> int:
    ...
    completed = subprocess.run(PREFLIGHT_COMMAND, check=False)
    if completed.returncode != 0:
        print(
            f"[Hunter Pre-Push] BLOCKED: canonical normal preflight exited {completed.returncode}",
            file=sys.stderr,
        )
    ...
    print(f"[Hunter Pre-Push] PASS: exact HEAD {after_head} passed canonical normal preflight")
    return 0
