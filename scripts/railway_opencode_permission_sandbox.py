...
namespaces used by the default provider sandbox. This shim accepts only the
...
def _parse(argv: list[str]) -> tuple[Path, Path, str, list[str]]:
    ...
    if not (host_executable.startswith("/app/bin/") or host_executable.startswith("/usr/") or host_executable.startswith("/bin/")):
